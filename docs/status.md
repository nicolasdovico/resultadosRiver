...
- [x] **Sección de Figuras (Jugadores):**
    ...
    - [x] **Sección de Técnicos (Normalización de Base de Datos y Multi-Ciclos):**
        ...
        - [x] **Top 3 Artilleros por Ciclo / Trayectoria (`top_scorers`):** Integración de componente visual de goleadores estilo Torneos en la tarjeta Hero de `/tecnicos/[id]`, mostrando avatares circulares superpuestos con fotos de jugadores, insignia de goles convertidos, enlaces directos a sus fichas y cómputo dinámico (Top 3 del ciclo seleccionado o Top 3 global acumulado).
        - [x] **Estandarización de Fichas de Partidos (`TecnicoMatches`):** Implementación del componente `TecnicoMatches` homologando el diseño visual de tarjetas de encuentros con `EstadioMatches` y `ArbitroMatches` (insignias de torneo, fase, fecha, condición, sede, escudos oficiales de River y Rival, marcador resaltado y distintivo G/E/P). Cada tarjeta es interactiva y redirige a `/partidos/[fecha]` para ver el detalle completo del encuentro, manteniendo la restricción de paywall (3 partidos + mask-fade-bottom) para usuarios Free y acceso completo con paginador de 15 partidos para socios Premium.
        - [x] **Testing:** Suite completa de tests en `tests/Feature/TecnicoApiTest.php` (4 tests pasando, 21 tests globales con 190 aserciones).
