...
interface TecnicoMatchesProps {
  partidos: Partido[];
  itemsPerPage?: number;
}

export default function TecnicoMatches({ partidos, itemsPerPage = 15 }: TecnicoMatchesProps) {
  const [currentPage, setCurrentPage] = useState(1);

  // Sort matches by date descending (standard for history)
    ...
    return (
      <div className="bg-white rounded-[48px] p-24 text-center border-2 border-dashed border-zinc-100">
        <p className="text-zinc-400 font-black uppercase text-xs tracking-[0.2em]">
          Sin registros históricos disponibles para este ciclo
        </p>
      </div>
    );
  }
          ...
          >
            <div className="flex-1">
              <div className="flex flex-wrap items-center gap-2 mb-4">
                ...
                {p.estadio && (
                  <span className="text-[10px] bg-zinc-50 text-zinc-600 px-2 py-0.5 rounded-full font-bold uppercase tracking-tight">
                    🏟️ {p.estadio.es_desc}
                  </span>
                )}
              </div>
