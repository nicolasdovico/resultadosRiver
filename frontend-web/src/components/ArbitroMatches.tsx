...
interface ArbitroMatchesProps {
  partidos: Partido[];
  itemsPerPage?: number;
}

export default function ArbitroMatches({ partidos, itemsPerPage = 15 }: ArbitroMatchesProps) {
  const [currentPage, setCurrentPage] = useState(1);

  // Sort matches by date descending (standard for referees history)
    ...
    return (
      <div className="bg-white rounded-[48px] p-24 text-center border-2 border-dashed border-zinc-100">
        <p className="text-zinc-400 font-black uppercase text-xs tracking-[0.2em]">
          Sin registros históricos disponibles con este árbitro
        </p>
      </div>
    );
  }
