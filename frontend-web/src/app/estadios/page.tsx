import { MapPin, Target, Zap, Search, X, Info, Star, Activity, ChevronRight, Trophy, Landmark } from "lucide-react";
...
export default async function EstadiosPage({
  ...
  let visibleEstadios: Estadio[] = [];
  let totalResults = 0;
  let lastPage = 1;

  if (query || letter) {
    const response = await customInstance<{ data: Estadio[], meta?: any }>({
      url: "/v1/estadios",
      method: "GET",
      params: { 
        q: query, 
    ...
    visibleEstadios = response.data || [];
    totalResults = response.meta?.total || visibleEstadios.length;
    lastPage = response.meta?.last_page || 1;
  }

  // Fetch Top 5 Stadiums where River played the most matches for the hero section
  let topEstadios: Estadio[] = [];
  if (!query && !letter) {
    try {
      const topResponse = await customInstance<{ data: Estadio[] }>({
        url: "/v1/estadios/top",
        method: "GET",
        ...fetchOptions
      });
      topEstadios = topResponse.data || [];
    } catch (e) {
      console.error("Error fetching top estadios", e);
    }
  }

  // Restriction logic for non-premium users (max 15)
  let shownCount = visibleEstadios.length;
  let isRestricted = false;

  if (isLoggedIn && !isPremium && (query || letter)) {
    const limit = 15;
    if (totalResults > limit) {
      if (visibleEstadios.length > limit) {
        visibleEstadios = visibleEstadios.slice(0, limit);
        shownCount = limit;
      } else {
        shownCount = visibleEstadios.length;
      }
      isRestricted = true;
    }
  }
  ...
  return (
    <div className="min-h-screen bg-zinc-50/50 pb-24">
            ...
            <div className="max-w-2xl">
              <div className="flex items-center space-x-3 mb-6">
                ...
                <h1 className="text-5xl font-black text-zinc-900 tracking-tighter uppercase italic">
                  Archivo de <span className="text-red-600">Estadios</span>
                </h1>
              </div>
              <p className="text-zinc-500 font-medium text-lg leading-relaxed">
                Los escenarios de la gloria. Donde River Plate desplegó su fútbol y forjó su historia a través de las décadas.
              </p>
            </div>
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        {/* Sedes Principales (Top 5 Estadios en donde más se jugó) */}
        {!query && !letter && topEstadios.length > 0 && (
          <section className="mb-20">
            <div className="flex items-center justify-between mb-10">
              <h2 className="text-3xl font-black text-zinc-900 tracking-tighter uppercase italic flex items-center">
                ...
                Sedes Principales (Donde Más se Jugó)
              </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
              {topEstadios.map((estadio, index) => {
                const diff = (estadio.stats?.pg || 0) - (estadio.stats?.pp || 0);
                  ...
                  >
                    <div>
                      <div className="flex items-center justify-between mb-4 relative z-10">
                        <div className={`w-8 h-8 rounded-xl flex items-center justify-center font-black text-xs ${
                          isMonumental ? 'bg-red-600 text-white' : 'bg-zinc-800 text-zinc-400 group-hover:text-white'
                        }`}>
                          #{index + 1}
                        </div>
                        {isMonumental ? (
                          ...
                          <span className="text-[9px] font-bold tracking-widest uppercase text-zinc-500">
                            {estadio.stats?.efectividad}% Efec.
                          </span>
                        )}
                      </div>
                    ...
                    <div className="relative z-10 pt-4 border-t border-white/5">
                      ...
                      <div className="flex items-center justify-between text-[10px] text-zinc-400 font-bold uppercase tracking-wider">
                        <span>{estadio.stats?.pg}G • {estadio.stats?.pe}E • {estadio.stats?.pp}P</span>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
            <div className="flex items-center space-x-4">
              <h2 className="text-3xl font-black text-zinc-900 tracking-tighter uppercase italic">
                {letter ? `Letra ${letter}` : query ? "Búsqueda" : "Explorar Sedes"}
              </h2>
              {(letter || query) && (
                <Link 
                  href="/estadios"
            ...
            <div className="relative w-full md:max-w-md">
              <SearchBar placeholder="Buscar estadio..." className="w-full" />
            </div>
          </div>
            ...
            <div className="mb-8 bg-yellow-50 border border-yellow-200 rounded-[32px] p-6 flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm shadow-yellow-900/5">
              <div className="flex items-center gap-4">
                ...
                <p className="text-yellow-900 font-medium text-sm md:text-base">
                  Mostrando <span className="font-black">{shownCount}</span> de <span className="font-black">{totalResults}</span> estadios.{" "}
              ...
              {visibleEstadios.length > 0 ? (
                <>
                    ...
                    {visibleEstadios.map((estadio) => (
                      ...
                      >
                        <div className="w-20 h-20 bg-zinc-50 rounded-full flex items-center justify-center text-zinc-300 font-black text-2xl mb-4 group-hover:bg-red-50 group-hover:text-red-600 transition-all uppercase relative overflow-hidden border-2 border-transparent group-hover:border-red-100">
                          <MapPin size={32} className="group-hover:scale-110 transition-transform duration-500" />
                        </div>
                  ...
                  {!isRestricted && lastPage > 1 && (
                    <div className="flex justify-center items-center space-x-2 mt-12">
                      {currentPage > 1 ? (
                        <Link 
                          href={`/estadios?${new URLSearchParams({ 
                            ...(query ? { q: query } : {}), 
                            ...(letter ? { letra: letter } : {}),
                ...
                <div className="text-center py-32 bg-white rounded-[48px] border-2 border-dashed border-zinc-100 shadow-inner">
                  ...
                  <p className="text-zinc-400 font-medium max-w-sm mx-auto">
                    Revisa el nombre o intenta con otro estadio.
                  </p>
                </div>
              )}
            </>
